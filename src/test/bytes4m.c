...
long lcd_ioctl(struct file* filep, unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd) {
    case LCD_IOC_HELLO:
      ...
      break;
    case LCD_IOC_WRITE:
      msg = (struct dev_message*) arg;
      copy_from_user(dev_msg.data, msg->data, 20);
      printk("write %s\n", dev_msg.data);
      break;
    case LCD_IOC_READ:
      msg = (struct dev_message*) arg;
      copy_to_user(msg->data, dev_msg.data, 20);
      break;
    case LCD_IOC_WR:
      msg = (struct dev_message*) arg;
      copy_from_user(tmp.data, msg->data, 20);
      copy_to_user(msg->data, dev_msg.data, 20);
      strncpy(dev_msg.data, tmp.data, 20);
      break;  
    default:
      return -ENOTTY;
  }
